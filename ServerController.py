...
class Email_Dispatch_Provider(View):
   methods = ['POST']  
   def dispatch_request(self) -> str :
      
        if request.method == 'POST':
            Name  = request.form.get('EmailAddr') 
            UseScenarios = request.form.get('UseCases')
            Interaction = request.form.get('Publications')
            print(Name , UseScenarios , Interaction )
            
            # Dispatch Email 
            base.Create_Email(Name , UseScenarios , Interaction )    

            # Book Email  To WaitList 
            #         
        else: 
            # Return requested profile thru client connect 
...
class Content_Timeline_Paradox_Concept(View):
   methods = ['GET']  
   def dispatch_request(self , AccountAddress  ) -> str :
        # ----------------------------------------------------------------------
        ...
        if request.method == 'GET':
            ...
            return render_template('Timeline_Paradox_Concept.html'   , AccountAddress = AccountAddress  , Current_Dateline = Current_Dateline , Current_Timeline =  Current_Timeline , UserToken = UserToken , Access_Username = Access_Username , Profile_Avatar =  Profile_Avatar , Creator_Listings = Creator_Listings , Creator_Index = Creator_Index ,  Storyline_Concept = Storyline_Concept ,  Storyline_Index = Storyline_Index ,  Nft_Tags = Nft_Tags , Nft_Tag_Index = Nft_Tag_Index ,  CompanyID = CompanyID)
        
        return render_template('Timeline_Paradox_Concept.html' ,  AccountAddress = AccountAddress  )
...
class Creators_Content_Paradox(View):
   ...
   def dispatch_request(self , AccountAddress ) -> str :
        ...
        if request.method == 'GET':
            ...
            if Creator_Records :
                Creator_Index = len(Creator_Records) 
            else: 
                Creator_Index = int(0)

            Profile_Avatar =  CWTInterface.Render_Profile_Avatar(AccountAddress) 

            return render_template('Creators-Paradox-Concept.html'   , AccountAddress = AccountAddress  , Creator_Records  = Creator_Records , Creator_Index = Creator_Index ,  Profile_Avatar = Profile_Avatar  )
...
app.add_url_rule('/Dashboard/Timeline/Account/<string:AccountAddress>/Hist/' , view_func = Content_Timeline_Paradox_Concept.as_view('Timeline'))
app.add_url_rule('/Email/Dispatch/Account/' , view_func = Email_Dispatch_Provider.as_view('Waitlist_Provider'))
